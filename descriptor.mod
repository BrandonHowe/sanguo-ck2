name="Romance of the Three Kingdoms Dev"
archive="mod/sanguo.zip"
replace_path="common/alternate_start"
replace_path="common/artifact_spawns"
replace_path="common/artifacts"
replace_path="common/bloodlines"
replace_path="common/bookmarks"
replace_path="common/buildings"
replace_path="common/cb_types"
replace_path="common/combat_tactics"
replace_path="common/council_positions"
replace_path="common/council_voting"
replace_path="common/cultures"
replace_path="common/customizable_localisation"
replace_path="common/death"
replace_path="common/death_text"
replace_path="common/defines"
replace_path="common/disease"
replace_path="common/dynasties"
replace_path="common/event_modifiers"
replace_path="common/execution_methods"
replace_path="common/game_rules"
replace_path="common/government_flavor"
replace_path="common/governments"
replace_path="common/heir_text"
replace_path="common/holding_types"
replace_path="common/job_actions"
replace_path="common/job_titles"
replace_path="common/landed_titles"
replace_path="common/laws"
replace_path="common/mercenaries"
replace_path="common/minor_titles"
replace_path="common/modifier_definitions"
replace_path="common/nicknames"
replace_path="common/objectives"
replace_path="common/offmap_powers"
replace_path="common/on_actions"
replace_path="common/opinion_modifiers"
replace_path="common/province_setup"
replace_path="common/religion_features"
replace_path="common/religion_modifiers"
replace_path="common/religions"
replace_path="common/religious_titles"
replace_path="common/retinue_subunits"
replace_path="common/save_conversion"
replace_path="common/scripted_effects"
replace_path="common/scripted_score_values"
replace_path="common/scripted_triggers"
replace_path="common/societies"
replace_path="common/special_troops"
replace_path="common/stories"
replace_path="common/succession_voting"
replace_path="common/trade_routes"
replace_path="common/traits"
replace_path="common/tributary_types"
replace_path="common/triggered_modifiers"
replace_path="common/wonder_upgrades"
replace_path="common/wonders"
replace_path="common/achievements.txt"
replace_path="common/defines.lua"
replace_path="common/defines.txt"
replace_path="common/hints.txt"
replace_path="common/static_modifiers.txt"
replace_path="common/technology.txt"
replace_path="dlc_metadata"
replace_path="history/characters"
replace_path="history/offmap_powers"
replace_path="history/provinces"
replace_path="history/technology"
replace_path="history/titles"
replace_path="history/wars"
replace_path="gfx/loadingscreens"
replace_path="gfx/flags"
replace_path="decisions"
replace_path="events"
replace_path="localisation/customizable_localisation"
replace_path="map"
replace_path="music"
user_dir="Sanguodev"
tags=
{
	Romance of Three Kingdoms Three Kingdoms Total Conversion 
}
picture="Sanguo.png"
